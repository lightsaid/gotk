...
func (s *ServeMux) Handle(pattern string, handler http.Handler, methods ...string) {
	...
	for _, method := range methods {
		err := s.routes.Insert(pattern, handler, method)
		if err != nil {
			panic(err)
		}
	}

	// if err := s.routes.Insert(pattern, handler, methods...); err != nil {
...
func (s *ServeMux) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// pattern := cleanPath(r.URL.Path)
	// // 路由匹配
	// ctx, match := s.routes.Match(pattern)
	// if !match {
